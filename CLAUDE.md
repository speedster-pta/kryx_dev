...
- **`storage/__init__.py` re-exports every name explicitly, by hand** (not via `__all__`) — its own docstring states this is so existing call sites (`from autosend import storage; storage.get_x(...)`) don't break. When adding a new function to any `storage/*.py` submodule that other code should call via `storage.`, add the explicit import to `storage/__init__.py` too; don't rely on `__all__` or wildcard imports.
...
No CI/CD. Deployment is manual rsync to a single VPS (SSH alias `shofar-cloud`):
