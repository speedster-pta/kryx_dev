...
def create_onboarding_intent(staff_user_id: int, unit_id: int) -> int:
    from datetime import datetime, timezone

    with _connect() as conn:
        cur = conn.execute(
            "INSERT INTO whatsapp_onboarding_intents (staff_user_id, unit_id, created_at) VALUES (?, ?, ?)",
            (staff_user_id, unit_id, datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()
        return cur.lastrowid


def consume_latest_onboarding_intent(staff_user_id: int, max_age_minutes: int) -> dict | None:
    ...
    with _connect() as conn:
        row = conn.execute(
            ...
            WHERE staff_user_id = ? AND consumed_at IS NULL AND created_at >= ?
            ORDER BY created_at DESC LIMIT 1
            """,
            (staff_user_id, cutoff),
        ).fetchone()
        if not row:
            return None
        ...
        # unlikely) race of two callbacks for the same staff_user_id
