...
Core schema only: organisations, organisation_modules, units, staff_users,
...
`organisations`, `organisation_modules`, `units`, and `staff_users`.
...
def init_core_schema(conn) -> None:
    ...
    _create_staff_users(conn)
    _create_staff_user_units(conn)
...
# ---------------------------------------------------------------------------
# staff_users — org-scoped staff, with a separate partial-unique path for
# platform super-admins (context seed §4.6) who span every organisation.
# ---------------------------------------------------------------------------

def _create_staff_users(conn) -> None:
    conn.execute(
        """
    ...
    conn.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS idx_staff_users_platform_admin_username
        ON staff_users(username) WHERE org_id IS NULL
        """
    )
    conn.execute("CREATE INDEX IF NOT EXISTS idx_staff_users_org_id ON staff_users(org_id)")


def _create_staff_user_units(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS staff_user_units (
            staff_user_id INTEGER NOT NULL REFERENCES staff_users(id) ON DELETE CASCADE,
            unit_id INTEGER NOT NULL REFERENCES units(id) ON DELETE CASCADE,
            PRIMARY KEY (staff_user_id, unit_id)
        )
        """
    )
...
def _create_whatsapp_onboarding_intents(conn) -> None:
    ...
    # state. Correlated back via staff_user_id (the callback runs in the
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_onboarding_intents_staff ON whatsapp_onboarding_intents(staff_user_id, consumed_at)"
    )
