...
from shofar_automation import crypto
from shofar_automation.storage import DB_PATH
...
class Unit(Base):
    ...
    # Moved to the org-wide PCOOrganizationSettings singleton below - PCO
    # credentials are org-level, not per-unit. pco_webhook_secret
...
class PCOOrganizationSettings(Base):
    """Org-wide PCO API credentials - singleton table, one row. Split out
...
class MetaPlatformSettings(Base):
    """Org-wide Meta app credentials for WhatsApp Embedded Signup -
    singleton table, one row, same shape/reasoning as
    PCOOrganizationSettings above. app_id and config_id aren't secret
    (already visible in the Embedded Signup URL itself); app_secret is a
    live credential, so it's encrypted."""
...
class StaffUser(Base):
    ...
    units = relationship("Unit", secondary=staff_user_units_table)
