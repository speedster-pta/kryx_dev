...
def get_ai_settings(whatsapp_number_id: int) -> dict | None:
    with _connect() as conn:
        row = conn.execute(
            "SELECT id, whatsapp_number_id, custom_instructions, bot_description, handoff_message, created_at "
            "FROM whatsapp_number_ai_settings WHERE whatsapp_number_id = ?",
            (whatsapp_number_id,),
        ).fetchone()
        if not row:
            return None
        columns = ["id", "whatsapp_number_id", "custom_instructions", "bot_description", "handoff_message", "created_at"]
    ...
    with _connect() as conn:
        conn.execute(
            ...
            INSERT INTO whatsapp_number_ai_settings
                (whatsapp_number_id, custom_instructions, bot_description, handoff_message, created_at)
            VALUES (?, ?, ?, ?, ?)
