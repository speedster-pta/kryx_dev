...
async def _sync_subdomain(org_id: int) -> None:
    """Best-effort: fetches the connected PCO account's own Church Center
    subdomain (verified live - see
    PlanningCenterClient.get_organization_info) and syncs it, overwriting
    whatever was there before - this is PCO's own authoritative answer,
    not a value orgs hand-maintain any more (there's no manual entry
    field left on the PCO Settings page). Failures here are logged and
    swallowed, same reasoning as _auto_create_webhooks below - a lookup
    hiccup shouldn't undo an otherwise successful OAuth connection."""
