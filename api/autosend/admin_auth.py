...
from autosend.admin_models import engine, User, user_units_table
...
# never defined anywhere (only the `user_units_table` Core
    ...
    with Session(engine) as session:
        ...
        unit_ids = [
            r[0] for r in session.execute(
                select(user_units_table.c.unit_id).where(
                    user_units_table.c.user_id == user.id
                )
            ).all()
        ]
...
# dropdowns (e.g. the unit/organisation picker) down to the current user's
# scope. scaffold_form has no `request` param, so a contextvar is the only
...
# reachable unit ids should call web.auth.resolve_unit_ids(session)
# instead of reading this tuple's unit_ids directly.
#
...
current_scope: contextvars.ContextVar[
    tuple[bool, bool, int | None, list[int]] | None
...
class AdminAuth(AuthenticationBackend):
    ...
    async def authenticate(self, request: Request) -> bool:
        ...
        current_scope.set((
            ...
            request.session.get("is_org_admin", False),
