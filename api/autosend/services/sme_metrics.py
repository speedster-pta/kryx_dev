...
send_log rows record source="sme_metrics" going forward - rows sent
before this module's rename still say "email_wa" (see
integrations/sme_metrics/__init__.py), an accurate historical record of
what actually happened at the time, left as-is rather than backfilled.
"""

from autosend import storage
from autosend.clients import resolve_whatsapp_client
from autosend.integrations.sme_metrics.providers import PROVIDERS, UnparseableEmail
...
def _record(unit: dict, status: str, *, phone=None, template_name=None, whatsapp_number_id=None,
    ...
    storage.record_send(
        unit_id=unit["id"], source="sme_metrics", status=status,
...
async def process_inbound_email(local_part: str, body_text: str, dedup_key: str) -> None:
    ...
    if not storage.is_enabled(integration["org_id"], storage.MODULE_SME_METRICS):
        logger.info(
            "[%s] Inbound email for %s/%s but sme_metrics module is disabled for this org - dropping",
    ...
    logger.info(
        "[%s] Sent %s WhatsApp to %s via sme_metrics/%s/%s",
